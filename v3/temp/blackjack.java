...
class rorCalc {
    public static void main(String[] args) throws IOException{
        ...
        int numSuccesses = 0;
        long numShoes = 0;
        int maxPoint = bankroll*Integer.parseInt(args[2]);
        System.out.println("getting shoes");
        BufferedReader lengthReader = new BufferedReader(new FileReader(new File("shoes.txt")));
        int length = 0;
        String line = lengthReader.readLine(); 
        for(;line != null; length++) line = lengthReader.readLine();
        //System.out.println(length);
        int[] shoeChanges = new int[length], shoeMaxes = new int[length], shoeMins = new int[length];
        BufferedReader reader = new BufferedReader(new FileReader(new File("shoes.txt")));
        for(int i = 0; i<length;i++) {
            line = reader.readLine();
            shoeChanges[i] = Integer.parseInt(line.substring(0, line.indexOf(",")));
            line = line.substring(line.indexOf(",")+1);
            shoeMaxes[i] = Integer.parseInt(line.substring(0, line.indexOf(",")));
            line = line.substring(line.indexOf(",")+1);
            shoeMins[i] = Integer.parseInt(line);
        }
        ...
        for(int i = 0; i<numTests;i++) {
            ...
            for(int shoes = 0;true; shoes++) {
                int shoeNum = (int)(Math.random()*shoeChanges.length);
                if(shoeMaxes[shoeNum]+money>maxPoint) {
                    numSuccesses++;
                    numShoes+=shoes;
                    break;
                } else if(shoeMins[shoeNum]+money<0) {
                    break;
                } else money+=shoeChanges[shoeNum];
                //if(shoes%1000 == 0) System.out.println(money);
            }
        }
...
public class blackjack {
    ...
    public static final int[][] betSpread = {{1,10}/**/,{1,25},{1,50},{1,80}/*,{1,4000},{1,6000},{1,8000},{1,10000}/**/};
    ...
    public static void main(String[] args) throws IOException{
        ...
        if(recordShoes)writer = new BufferedWriter(new FileWriter(new File("shoes.txt")));
        for(int i = 0; i<numShoes; i++) {
            if(numShoes>=100)if(i%(numShoes/100) == 0) System.out.println(i/(numShoes/100)+"%");
            Shoe shoe = new Shoe();
            //lastBet = 0;
