...
class getMinLength {
    public static void main(String[] args) throws IOException{
        BufferedReader br = new BufferedReader(new FileReader(new File("shoes.txt")));
        int minLength = 10000;
...
class rorCalc {
    public static void main(String[] args) throws IOException{
        double desiredROR = Double.parseDouble(args[0]);
        int numTests = Integer.parseInt(args[1]);
        int[] maxMoneyForBetspread = new int[args.length-2];
        for(int i = 0; i<maxMoneyForBetspread.length; i++) maxMoneyForBetspread[i] = Integer.parseInt(args[i+2]);
        int maxPoint = maxMoneyForBetspread[maxMoneyForBetspread.length-1];
        System.out.println("getting shoes");
        //System.out.println(length);
        int[][] shoeChanges = new int[maxMoneyForBetspread.length][], shoeMaxes = new int[maxMoneyForBetspread.length][], shoeMins = new int[maxMoneyForBetspread.length][];
        ...
        int[] lowestPoints = new int[numTests];
        int timesBelow1870 = 0;
        for(int i = 0; i<numTests;i++) {
            if(numTests>=100)if(i%(numTests/10) == 0) System.out.println(i/(numTests/100)+"%");
            int money = 0;
            int betSpread = 0;
            while(maxMoneyForBetspread[betSpread]<money){
                betSpread++;
                if(maxMoneyForBetspread.length-1 == betSpread) break;
            }
            for(int shoes = 0;true; shoes++) {
                ...
                if(shoeMaxes[betSpread][shoeNum]+money>maxPoint) {
                    break;
                } else if(shoeMins[betSpread][shoeNum]+money<lowestPoints[i]) {
                    lowestPoints[i] = shoeMins[betSpread][shoeNum];
                }
                money+=shoeChanges[betSpread][shoeNum];
                if(betSpread < maxMoneyForBetspread.length-1) {
                    if(money>maxMoneyForBetspread[betSpread]) betSpread++;
                }
                if(betSpread>0) {
                    if(money<maxMoneyForBetspread[betSpread-1]) betSpread--;
                }
            }
            if(lowestPoints[i]<-1870) timesBelow1870++;
        }
        Arrays.sort(lowestPoints);
        System.out.println((double)timesBelow1870/numTests);
        System.out.println("bankroll required: "+(-lowestPoints[(int)(numTests*desiredROR)]));
    }
...
public class blackjack {
    ...
    public static final int maxSplitHands = 3;
    ...
    public static int[][] betSpread = {{1,10},{1,25},{1,100}};
    ...
    public static void main(String[] args) throws IOException{
        ...
        for(int i = 75; i<=300; i+=25) {
            ...
            numHands = 0;
            lastBet = 0;
            System.out.println("top bet: "+i);
            simShoes(Arrays.copyOfRange(args,0,4));
            rorCalc.main(Arrays.copyOfRange(args,4,args.length));
        }
    }
    public static void simShoes(String[] args) throws IOException{
       ...
       // if(args.length>4) debug = Boolean.parseBoolean(args[4]);
        ...
        for(int i = 0; i<numShoes; i++) {
            if(numShoes>=100)if(i%(numShoes/10) == 0) System.out.println("betspread "+betSpreadNum+": "+(i/(numShoes/100))+"%");
            Shoe shoe = new Shoe();
            //lastBet = 0;
...
// class SortedArrayList<E extends Comparable> {
//     public E[] sortedArray;

// }
// class TreeList<E extends Comparable> {
//     public TreeList<E> left, right, parent;
//     public E value;
//     public int numLeft, numRight;
//     public TreeList(E element, TreeList<E> parent) {
//         this.parent = parent;
//         this.value = element;
//         numLeft = 0;
//         numRight = 0;
...
//         right = null;
//     }
//     public E getPercentile(double percentile) {
//         double thisPercentile = (numLeft+1.0)/(numLeft+numRight+1.0);
//         if(thisPercentile>percentile) {
//             if(left == null) return value;
//             else return left.getPercentile(percentile/thisPercentile);
//         } else if(thisPercentile<percentile) {
//             if(right == null) return value;
//             else return right.getPercentile((percentile-thisPercentile)/(1-thisPercentile))
//         } else {
//             return value;
//         }
//     }
...
//         }
//     }
//     void addToRight(E element) {
//         if(right == null) {
//             right = new TreeList<E>(element, this);
//         } else {
//             right.add(element);
//         }
//         numRight++;
//     }
//     void addToLeft(E element) {
//         if(left == null) {
//             left = new TreeList<E>(element, this);
//         } else {
//             left.add(element);
//         }
//         numLeft++;
